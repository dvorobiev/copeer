...
def load_previous_state(state_file):
    if os.path.exists(state_file):
        with open(state_file,newline='',encoding='utf-8') as f:copied_files.update(str(Path(row[0]))for row in csv.reader(f)if row)
...
def save_session(files,source_root):
    session_data={'files_to_copy':files,'source_root':source_root};
...
def get_files_to_copy():
    ...
    if choice=='1':
        ...
        source_root=console.input("Введите путь к каталогу для копирования: ").strip()
        files_to_copy=[str(p)for p in Path(source_root).rglob("*")if p.is_file()and not p.is_symlink()]
    ...
    return files_to_copy,source_root
...
def main():
    ...
    files_to_process = [str(Path(f)) for f in files_to_copy if str(Path(f)) not in copied_files]
    if not files_to_process:
        console.print("[bold green]✅ Все файлы из выбранной сессии уже скопированы. Завершение.[/bold green]"); clear_session(); return

    total_files_to_process = len(files_to_process)
    console.print(f"Всего файлов в задании: [bold]{len(files_to_copy)}[/bold]. Уже скопировано: [bold]{len(copied_files)}[/bold]. Осталось: [bold green]{total_files_to_process}[/bold green]")

    disk_manager = DiskManager(config['mount_points'], config['threshold'])

    # --- ИЗМЕНЕНИЕ: Создаем TextColumn для счетчика файлов отдельно ---
    ...
    progress = Progress(
        ...
        TaskProgressColumn(),
        TextColumn("•"),
        file_counter_column,  # Вставляем наш управляемый виджет
        TextColumn("•"),
        TransferSpeedColumn(),
        TextColumn("•"),
        TimeRemainingColumn(),
        ...
        with Live(layout, screen=True, redirect_stderr=False, vertical_overflow="visible") as live:
            with ThreadPoolExecutor(max_workers=config['threads']) as executor:
                ...
                for future in as_completed(future_to_file):
                    ...
                    # Обновляем прогресс по байтам
                    progress.update(main_task, advance=size_copied)
                    # Обновляем текст в нашем счетчике файлов
                    file_counter_column.text_format = f"[cyan]{files_completed_count}/{total_files_to_process} файлов[/cyan]"

                    layout["top"].update(generate_disks_panel(disk_manager, config))
                    layout["middle"].update(generate_workers_panel())
    except (Exception, KeyboardInterrupt):
        console.print("\n[bold red]Копирование прервано пользователем или из-за критической ошибки.[/bold red]"); console.print("[yellow]Сессия сохранена. При следующем запуске вы сможете продолжить.[/yellow]"); sys.exit(1)

    if copy_successful and progress.finished:
        console.rule("[bold green]✅ Копирование успешно завершено[/bold green]"); clear_session()
    else:
        console.rule("[bold yellow]Копирование завершено, но были ошибки. Проверьте лог.[/bold yellow]"); console.print("[yellow]Сессия сохранена. Вы можете перезапустить скрипт, чтобы попробовать скопировать проблемные файлы снова.[/yellow]")
